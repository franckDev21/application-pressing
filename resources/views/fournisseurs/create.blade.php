<x-dashboard-layout>
  <h1 class="text-2xl border-b pb-4 font-extrabold text-gray-500">
    <span>#Ajouter d'un nouveau  </span>
    <span class="text-cyan-500">fournisseur</span>
  </h1>

  <form action="{{ route('fournisseur.store') }}" method="POST" class="bg-white pt-4 pb-6 rounded-lg">
    @csrf
    <div class="flex px-3">
      <div class="w-1/2 px-2">
        <label for="" class="mb-3 inline-block text-gray-500">Nom du fournisseur</label>
      ...
      <div class="w-1/2 px-2">
        <label for="" class="mb-3 inline-block text-gray-500">adresse du fournisseur</label>
        <input name="address" value="{{ old('address') }}" type="text" placeholder="Adresse" class="border-2 border-gray-200 outline-none focus:ring-0 focus:outline-none focus:border-gray-200 py-2 w-full bg-gray-50  rounded-md ">
