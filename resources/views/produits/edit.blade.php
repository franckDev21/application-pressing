<x-dashboard-layout>
  <h1 class="text-2xl border-b pb-4 font-extrabold text-gray-500">
    <span>#Edité le produit </span>
    <span class="text-cyan-500">{{ $produit->nom }}</span>
  </h1>

  <form action="{{ route('produit.update',$produit) }}" method="POST" class="bg-white pt-4 pb-6 rounded-lg">
    @csrf
    @method('PATCH')
    <div class="flex px-3">
      <div class="w-1/3 px-2">
        <label for="" class="mb-3 inline-block text-gray-500">Nom du produit</label>
        <input name="nom" value="{{ old('nom',$produit->nom) }}" type="text" placeholder="Nom" class="border-2 border-gray-200 outline-none focus:ring-0 focus:outline-none focus:border-gray-200 py-2 w-full bg-gray-50  rounded-md ">
      ...
      <div class="w-1/3 px-2">
        ...
        <input name="quantite" min="0" value="{{ old('quantite',$produit->quantite) }}" type="number" placeholder="quantité" class="border-2 border-gray-200 outline-none focus:ring-0 focus:outline-none focus:border-gray-200 py-2 w-full bg-gray-50  rounded-md ">
      ...
      <div class="w-1/3 px-2">
        ...
        <select name="fournisseur_id" placeholder="Fournisseur" class="border-2 px-3 border-gray-200 outline-none focus:ring-0 focus:outline-none focus:border-gray-200 py-2 w-full bg-gray-50  rounded-md ">
          @foreach ($fournisseurs as $fournisseur)
            <option @selected(old('fournisseur_id',$produit->fournisseur->id) == $fournisseur->id) value="{{ $fournisseur->id}}">{{ $fournisseur->nom }}</option>
           @endforeach
        </select>
    ...
    <div class="flex px-3 mt-4">
      <div class="w-1/2 px-2">
        ...
        <select name="unite" placeholder="unite" class="border-2 px-3 border-gray-200 outline-none focus:ring-0 focus:outline-none focus:border-gray-200 py-2 w-full bg-gray-50  rounded-md " id="">
          <option @selected(old('unite',$produit->unite) === 'G' ) value="G">Gramme</option>
          <option @selected(old('unite',$produit->unite) === 'KG' ) value="KG">Kilogramme</option>
          <option @selected(old('unite',$produit->unite) === 'L' ) value="L">Littre</option>
        </select>
      ...
      <div class="w-1/2 px-2">
        ...
        <div class="text-red-400">@error('prix_achat',$produit->prix_achat) {{ $message }} @enderror</div>
      </div>
    </div>

    <div class="text-center mt-6">
      <button class="px-6 rounded-md font-bold py-2 bg-cyan-600 text-white">Modifié</button>
    </div>
  </form>
</x-dashboard-layout>
